...
public class AddWordsList extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String newwords = request.getParameter("words");
		
		
		if(type.contains("w")){
			WordLists wl = new WordLists();
			
			wl.AddWordsList(newwords, listID);
		...
		int likesSize;
		ArrayList<String> list = ul.getAllFromList(Integer.parseInt(listID));	
       ...
       for(int i = 0; i < words.size(); i ++){
		...
		request.setAttribute("idlist", Integer.parseInt(listID));
